...
from backend.models.models import Source, Item, Favorite, Job, JobPart, ResolverAlias, GameMetadata
                    ...
                    if targets:
                        for target_g in targets:
                            if not target_g.get("appId"): # Don't overwrite if found multiple ways
                                ...
                                applied_count += 1
                if applied_count > 0:
                    print(f"[Library] Recognition: Restaurados {applied_count} AppIDs do banco.")
        finally:
            session.close()
